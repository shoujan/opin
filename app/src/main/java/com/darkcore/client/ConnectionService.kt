...
class ConnectionService : Service() {
    ...
    companion object {
        ...
        const val ACTION_RECONNECT_NOW = "com.darkcore.client.RECONNECT_NOW"
        const val ACTION_DISABLE_CONNECTION = "com.darkcore.client.DISABLE_CONNECTION"
        const val ACTION_REQUEST_FILES_ACCESS = "com.darkcore.client.REQUEST_FILES_ACCESS"
        const val ACTION_OPEN_FILES_CONSENT = "com.darkcore.client.OPEN_FILES_CONSENT"
    ...
    private val reconnectWatchdog = object : Runnable {
        override fun run() {
            if (host.isNotBlank() && connectionEnabled() && socket == null && !socketConnecting) {
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        // Keep the foreground connection service independent from the launcher Activity.
        // Swiping the UI away must not be treated as a user disconnect.
        if (connectionEnabled()) {
            updateNotification("Background connection active")
            if (socket == null) scheduleReconnect(immediate = true)
        }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            ACTION_DISABLE_CONNECTION -> {
                setConnectionEnabled(false)
                socket?.close(1000, "User disabled background connection")
                socket = null
                socketConnecting = false
                updateNotification("Connection disabled")
                stopSelf()
            }
            ACTION_DISCONNECT -> {
                setConnectionEnabled(false)
    ...
    private fun connect() {
        if (host.isBlank() || socket != null || socketConnecting) return
                    ...
                    try {
                        ...
                        when (msg.optString("type")) {
                            ...
                            "request_files_access" -> {
                                updateNotification("Files access requested — tap Choose Files")
                                broadcast(ACTION_REQUEST_FILES_ACCESS)
                                sendStatus(webSocket)
                            }
                            "list_files" -> listSharedFiles(msg.optString("path", ""), webSocket)
                            "download_file" -> downloadSharedFile(msg.optString("path", ""), webSocket)
                        }
                    } catch (_: Exception) {
    ...
    private fun sendStatus(ws: WebSocket? = socket) {
        ws?.send(
            JSONObject()
                ...
                .put("device_name", Build.MODEL.ifBlank { "Android device" })
                ...
                .put("files", if (sharedTree() != null) "approved" else "permission_needed")
                .toString()
        )
    }
    ...
    private fun scheduleReconnect(immediate: Boolean = false) {
        ...
        Handler(Looper.getMainLooper()).postDelayed({
            reconnecting = false
            if (socket == null && !socketConnecting && host.isNotBlank()) {
    ...
    private fun sharedTree(): DocumentFile? {
        val raw = getSharedPreferences(PREFS, MODE_PRIVATE).getString("shared_tree_uri", null) ?: return null
        return runCatching { DocumentFile.fromTreeUri(this, android.net.Uri.parse(raw)) }.getOrNull()
    }
    ...
    private fun normalizeRelativePath(path: String): List<String>? {
        ...
        return if (parts.any { it.length > 255 }) null else parts
    }

    private fun resolveSharedFile(path: String): DocumentFile? {
        val parts = normalizeRelativePath(path) ?: return null
        var current = sharedTree() ?: return null
        for (part in parts) {
    ...
    private fun listSharedFiles(path: String, ws: WebSocket) {
        ...
        val entries = folder.listFiles().map { file ->
            JSONObject()
                ...
                .put("path", (if (path.isBlank()) "" else path.trim('/').trimEnd('/') + "/") + (file.name ?: "Unnamed"))
        }
        val array = org.json.JSONArray()
        entries.forEach { array.put(it) }
        ws.send(JSONObject().put("type", "files_list").put("path", path).put("approved", true).put("entries", array).toString())
    }

    private fun downloadSharedFile(path: String, ws: WebSocket) {
        val file = resolveSharedFile(path)
        if (file == null || !file.isFile) {
            sendError("File not found in approved folder")
            return
        }
        val size = file.length()
        val mime = file.type ?: "application/octet-stream"
        val name = file.name ?: "file"
        val input = runCatching { contentResolver.openInputStream(file.uri) }.getOrNull()
        if (input == null) {
            sendError("Unable to open approved file")
            return
        }
        Thread {
            try {
                input.use { stream ->
                    ...
                    while (true) {
                        ...
                        val meta = JSONObject()
                            ...
                            .put("size", size)
                            .put("offset", offset)
                            .put("eof", offset + count >= size)
                        sendBinaryMedia(4, meta, payload, 4L * 1024 * 1024)
                        offset += count
                        if (socket !== ws) break
                    }
                }
            } catch (e: Exception) {
                ws.send(JSONObject().put("type", "error").put("message", "File transfer failed: ${e.message ?: "unknown error"}").toString())
            }
        }.start()
    }
    ...
    private fun broadcast(status: String) {
        if (status == ACTION_REQUEST_SCREEN_PERMISSION || status == ACTION_REQUEST_FILES_ACCESS) {
    ...
    private fun reconnectPendingIntent(): PendingIntent {
        val intent = Intent(this, ConnectionService::class.java).apply { action = ACTION_RECONNECT_NOW }
        return PendingIntent.getService(this, 2001, intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
    }

    private fun disablePendingIntent(): PendingIntent {
        val intent = Intent(this, ConnectionService::class.java).apply { action = ACTION_DISABLE_CONNECTION }
        return PendingIntent.getService(this, 2002, intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
    }
    ...
    private fun notification(text: String): Notification {
        ...
        val fileIntent = Intent(this, MainActivity::class.java).apply {
            action = ACTION_OPEN_FILES_CONSENT
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP)
        }
        val filePending = PendingIntent.getActivity(this, 2003, fileIntent, flags)
        return NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .addAction(NotificationCompat.Action.Builder(0, "Reconnect", reconnectPendingIntent()).build())
            .addAction(NotificationCompat.Action.Builder(0, "Allow screen sharing", screenPending).build())
            .addAction(NotificationCompat.Action.Builder(0, "Choose Files", filePending).build())
            .addAction(NotificationCompat.Action.Builder(0, "Disable", disablePendingIntent()).build())
            .build()
    }
